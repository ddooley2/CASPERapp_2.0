...
class ot_parser():
    ...
    """
    def get_data(self, ot_data_path, cspr_file_path, aligned_fasta_file, sequence_data):
        self.offdict.clear()
        ...
        self.write_data(cspr_file_path, org_dict, sequence_data)
    ...
    """
    def write_data(self, cspr_path, distance_scores, sequence_data):
        ...
        with open(self.appended_file, 'w+') as data_output:
            data_output.write("Sequence,Off-Target Score,Organism,Distance,Gene Name,Location,PAM,Strand,On-Target Score\n")
            for key, values in self.offdict.items():
                added_values = sequence_data[key]
                # just output the sequence and the 0 if no hits
                if float(values[0]) == 0.0000000:
                    data_output.write(str(key) + ',' + '0' + ',,,' + added_values[0] + ',' + added_values[3] + ',' + added_values[4] + ',' + added_values[5] + ',' + added_values[2] + '\n')
                # if there are hits, output the organism as well, as well as every hit
                    ...
                    for i in range(len(values)):
                        data_output.write(str(key) + ',' + str(values[i]) + ',' + str(chrom_values[i]) + ',' + str(distance_scores[chrom_values[i]]) +',' + added_values[0] + ',' + added_values[3] + ',' + added_values[4] + ',' + added_values[5] + ',' + added_values[2] + '\n')
